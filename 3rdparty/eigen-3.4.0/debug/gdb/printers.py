...
class EigenSparseMatrixPrinter:
    ...
    class _iterator(_MatrixEntryIterator):
        def __init__(self, rows, cols, val, rowMajor):
            super(EigenSparseMatrixPrinter._iterator, self).__init__(rows, cols,
                                                                     rowMajor)
        ...
        def __next__(self):
            ...
            end = ((start + self.val['m_innerNonZeros'][outer]) if self.val[
                'm_innerNonZeros'] else
            ...
            if start >= end:
                ...
                indices = [data['m_indices'][x] for x in
    ...
    def to_string(self):
        ...
        if self.data:
            status = ("not compressed" if self.val[
...
def build_eigen_dictionary():
    ...
    pretty_printers_dict[re.compile('^Eigen::Matrix<.*>$')] = lambda \
            val: EigenMatrixPrinter("Matrix", val)
    pretty_printers_dict[re.compile('^Eigen::SparseMatrix<.*>$')] = lambda \
            val: EigenSparseMatrixPrinter(val)
    pretty_printers_dict[re.compile('^Eigen::Array<.*>$')] = lambda \
            val: EigenMatrixPrinter("Array", val)
