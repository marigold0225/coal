<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="/Site">
        ...
        <xsl:variable name="ErrorCount">
            <xsl:value-of select="count(//TestList/Test[@Status='error'])"/>
        </xsl:variable>
        <xsl:variable name="FailureCount">
            <xsl:value-of select="count(//Testing/Test[@Status='failed'])"/>
        </xsl:variable>
        <testsuite name="{$Name}" hostname="{$Hostname}" errors="0"
                   failures="{$FailureCount}" tests="{$TestCount}">
    ...
    <xsl:template match="Testing/Test">
        ...
        <xsl:variable name="testclassname">
            <xsl:value-of select=" concat('this', substring(Path,2))"/>
        </xsl:variable>
        ...
        <testcase name="{$testcasename}" classname="{$testclassname}"
                  time="{$exectime}">
